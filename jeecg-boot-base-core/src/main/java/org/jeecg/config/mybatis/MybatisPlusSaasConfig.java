...
public class MybatisPlusSaasConfig {
	...
	/**
	 * 是否开启系统模块的租户隔离 控制范围：用户、角色、部门、我的部门、字典、分类字典、多数据源、职务、通知公告
	 *
	 * 实现功能 1.用户表通过硬编码实现租户ID隔离 2.角色、部门、我的部门、字典、分类字典、多数据源、职务、通知公告除了硬编码还加入的 TENANT_TABLE
	 * 配置中，实现租户隔离更安全 3.菜单表、租户表不做租户隔离 4.通过拦截器MybatisInterceptor实现，增删改查数据 自动注入租户ID
	 */
