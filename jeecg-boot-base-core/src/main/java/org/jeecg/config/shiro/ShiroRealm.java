...
public class ShiroRealm extends AuthorizingRealm {
	...
	/**
	 * 用户信息认证是在用户进行登录的时候进行验证(不存redis) 也就是说验证用户输入的账号和密码是否正确，错误抛出异常
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		...
		if (token == null) {
			...
			log.info("————————身份认证失败——————————IP地址:  " + oConvertUtils.getIpAddrByRequest(req) + "，URL:"
					+ req.getRequestURI());
	 ...
	 * 4、当该用户这次请求jwt在生成的token值已经超时，并在cache中不存在对应的k，则表示该用户账户空闲超时，返回用户信息已失效，请重新登录。 注意：
	 * 前端请求Header中设置Authorization保持不变，校验有效性以缓存中的token为准。 用户过期时间 = Jwt有效时间 * 2。
	 * @param userName
	 * @param passWord
	 * @return
	 */
