...
public class DictAspect {
	...
	/**
	 * 本方法针对返回对象为Result 的IPage的分页列表数据进行动态字典注入 字典注入实现 通过对实体类添加注解@dict
	 * 来标识需要的字典内容,字典分为单字典code即可 ，table字典 code table text配合使用与原来jeecg的用法相同 示例为SysUser
	 * 字段为sex 添加了注解@Dict(dicCode = "sex") 会在字典服务立马查出来对应的text
	 * 然后在请求list的时候将这个字典text，已字段名称加_dictText形式返回到前端 例输入当前返回值的就会多出一个sex_dictText字段 { sex:1,
	 * sex_dictText:"男" } 前端直接取值sext_dictText在table里面无需再进行前端的字典转换了 customRender:function
	 * (text) { if(text==1){ return "男"; }else if(text==2){ return "女"; }else{ return
	 * text; } } 目前vue是这么进行字典渲染到table上的多了就很麻烦了 这个直接在服务端渲染完成前端可以直接用
	 * @param result
	 */
	private Object parseDictText(Object result) {
		if (result instanceof Result) {
			if (((Result) result).getResult() instanceof IPage) {
				...
				// update-begin--Author:zyf -- Date:20220606 ----for：【VUEN-1230】
				// 判断是否含有字典注解,没有注解返回-----
				...
				// update-end--Author:zyf -- Date:20220606 ----for：【VUEN-1230】
				// 判断是否含有字典注解,没有注解返回-----
				for (Object record : records) {
					String json = "{}";
					try {
						// update-begin--Author:zyf -- Date:20220531
						// ----for：【issues/#3629】 DictAspect Jackson序列化报错-----
						// 解决@JsonFormat注解解析不了的问题详见SysAnnouncement类的@JsonFormat
						json = objectMapper.writeValueAsString(record);
						// update-end--Author:zyf -- Date:20220531 ----for：【issues/#3629】
						// DictAspect Jackson序列化报错-----
					}
					catch (JsonProcessingException e) {
						log.error("json解析失败" + e.getMessage(), e);
					}
					// update-begin--Author:scott -- Date:20211223
					// ----for：【issues/3303】restcontroller返回json数据后key顺序错乱 -----
					JSONObject item = JSONObject.parseObject(json, Feature.OrderedField);
					// update-end--Author:scott -- Date:20211223
					// ----for：【issues/3303】restcontroller返回json数据后key顺序错乱 -----

					// update-begin--Author:scott -- Date:20190603
					// ----for：解决继承实体字段无法翻译问题------
					...
					for (Field field : oConvertUtils.getAllFields(record)) {
						...
						// update-end--Author:scott -- Date:20190603
						// ----for：解决继承实体字段无法翻译问题------
						...
						// HH:mm:ss");
						// item.put(field.getName(), aDate.format(new Date((Long)
						// item.get(field.getName()))));
						// }
						// update-end--Author:zyf -- Date:20220531 ----for：【issues/#3629】
						// DictAspect Jackson序列化报错-----
					}
				...
				for (JSONObject record : items) {
					for (Field field : dictFieldList) {
						...
						if (oConvertUtils.isNotEmpty(value)) {
							...
							log.debug(" __翻译字典字段__ " + field.getName() + CommonConstant.DICT_TEXT_SUFFIX + "： "
									+ textValue);
	...
	/**
	 * 一次性把所有的字典都翻译了 1. 所有的普通数据字典的所有数据只执行一次SQL 2. 表字典相同的所有数据只执行一次SQL
	 * @param dataListMap
	 * @return
	 */
	private Map<String, List<DictModel>> translateAllDict(Map<String, List<String>> dataListMap) {
		...
		for (String dictCode : dataListMap.keySet()) {
			...
			if (needTranslDataTable.size() > 0) {
				...
				for (DictModel dict : texts) {
					String redisKey = String.format("sys:cache:dictTable::SimpleKey [%s,%s]", dictCode,
							dict.getValue());
					try {
						// update-begin-author:taoyan date:20211012 for: 字典表翻译注解缓存未更新
						// issues/3061
						// 保留5分钟
						redisTemplate.opsForValue().set(redisKey, dict.getText(), 300, TimeUnit.SECONDS);
						// update-end-author:taoyan date:20211012 for: 字典表翻译注解缓存未更新
						// issues/3061
					}
					catch (Exception e) {
		...
		if (needTranslData.size() > 0) {
			...
			List<String> filterDictCodes = dictCodeList.stream()
	...
	private String translateDictValue(String code, String text, String table, String key) {
		...
		for (String k : keys) {
			...
			// update-begin--Author:scott -- Date:20210531 ----for： !56
			// 优化微服务应用下存在表字段需要字典翻译时加载缓慢问题-----
			if (!StringUtils.isEmpty(table)) {
				log.debug("--DictAspect------dicTable=" + table + " ,dicText= " + text + " ,dicCode=" + code);
				String keyString = String.format("sys:cache:dictTable::SimpleKey [%s,%s,%s,%s]", table, text, code,
						k.trim());
				if (redisTemplate.hasKey(keyString)) {
					try {
			...
			// update-end--Author:scott -- Date:20210531 ----for： !56
			// 优化微服务应用下存在表字段需要字典翻译时加载缓慢问题-----
