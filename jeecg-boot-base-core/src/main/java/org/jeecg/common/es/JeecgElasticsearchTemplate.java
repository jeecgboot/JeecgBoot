...
public class JeecgElasticsearchTemplate {
	...
	public boolean createIndex(String indexName) {
		...
		/*
		 * 返回结果 （仅供参考） "createIndex": { "shards_acknowledged": true, "acknowledged": true,
		 * "index": "hello_world" }
		 */
		try {
	...
	public JSONObject getIndexMapping(String indexName, String typeName) {
		 ...
		 * 参考返回JSON结构：
		 *
		 * { // 索引名称 "[indexName]": { "mappings": { // 分类名称 "[typeName]": { "properties":
		 * { // 字段名 "input_number": { // 字段类型 "type": "long" }, "input_string": { "type":
	...
	public boolean saveOrUpdate(String indexName, String typeName, String dataId, JSONObject data) {
		String url = this.getBaseUrl(indexName, typeName)
			.append("/")
			.append(dataId)
			.append("?refresh=wait_for")
			.toString();
		/*
		 * 返回结果（仅供参考） "createIndexA2": { "result": "created", "_shards": { "total": 2,
		 * "successful": 1, "failed": 0 }, "_seq_no": 0, "_index": "test_index_1",
		 * "_type": "test_type_1", "_id": "a2", "_version": 1, "_primary_term": 1 }
		 */
	...
	public boolean saveBatch(String indexName, String typeName, JSONArray dataList) {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			// 该行的操作
			// {"create": {"_id":"${id}", "_index": "${indexName}", "_type":
			// "${typeName}"}}
	...
	public boolean delete(String indexName, String typeName, String dataId) {
		String url = this.getBaseUrl(indexName, typeName).append("/").append(dataId).toString();
		/*
		 * 返回结果（仅供参考） { "_index": "es_demo", "_type": "docs", "_id": "001", "_version": 3,
		 * "result": "deleted", "_shards": { "total": 1, "successful": 1, "failed": 0 },
		 * "_seq_no": 28, "_primary_term": 18 }
		 */
		try {
	 ...
	 * @return { "range" : { field : { 『 "gt『e』?containMin" : min 』?min!=null , 『
	 * "lt『e』?containMax" : max 』}} }
	 */
