...
public abstract class AbstractQueryBlackListHandler {
	...
	/**
	 * key-表名 value-字段名，多个逗号隔开 两种配置方式-- 全部配置成小写 ruleMap.put("sys_user",
	 * "*")sys_user所有的字段不支持查询 ruleMap.put("sys_user",
	 * "username,password")sys_user中的username和password不支持查询
	 */
	...
	/**
	 * 根据 sql语句 获取表和字段信息，需要到具体的实现类重写此方法- 不同的场景 处理可能不太一样 需要自定义，但是返回值确定
	 * @param sql
	 * @return
	 */
	...
	protected class QueryTable {
		...
		public String toString() {
			return "QueryTable{" + "name='" + name + '\'' + ", alias='" + alias + '\'' + ", fields=" + fields + ", all="
					+ all + '}';
		}
