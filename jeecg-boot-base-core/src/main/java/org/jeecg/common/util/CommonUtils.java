...
public class CommonUtils {
	...
	/**
	 * 文件名 正则字符串 文件名支持的字符串：字母数字中文.-_()（） 除此之外的字符将被删除
	 */
	...
	public static String getFileName(String fileName) {
		...
		// update-beign-author:taoyan date:20220302 for: /issues/3381 online 在线表单
		// 使用文件组件时，上传文件名中含%，下载异常
		fileName = fileName.replaceAll(FILE_NAME_REGEX, "");
		// update-end-author:taoyan date:20220302 for: /issues/3381 online 在线表单
		// 使用文件组件时，上传文件名中含%，下载异常
		return fileName;
	}
	...
	public static DataSourceProperty getDataSourceProperty(String sourceKey) {
		DynamicDataSourceProperties prop = SpringContextUtils.getApplicationContext()
			.getBean(DynamicDataSourceProperties.class);
			...
			try {
				...
				if (dbType.indexOf(DataBaseConstant.DB_TYPE_MYSQL) >= 0) {
					DB_TYPE = DataBaseConstant.DB_TYPE_MYSQL;
						...
						|| dbType.indexOf(DataBaseConstant.DB_TYPE_DM) >= 0) {
