...
public class FreemarkerParseFactory {
	...
	/** 使用内嵌的(?ms)打开单行和多行模式 */
	private final static Pattern NOTES_PATTERN = Pattern.compile("(?ms)/\\*.*?\\*/|^\\s*//.*?$");
	...
	static {
		...
		// update-begin-author:taoyan date:2022-8-10 for: freemarker模板注入问题
		...
		// update-end-author:taoyan date:2022-8-10 for: freemarker模板注入问题
		// 禁止解析ObjectConstructor，Execute和freemarker.template.utility.JythonRuntime。
	}
	...
	public static boolean isExistTemplate(String tplName) throws Exception {
		try {
			...
			// update-begin--Author:scott Date:20180320 for：解决问题 - 错误提示sql文件不存在，实际问题是sql
			// freemarker用法错误-----
			...
			// update-end--Author:scott Date:20180320 for：解决问题 - 错误提示sql文件不存在，实际问题是sql
			// freemarker用法错误------
			return false;
		}
		return true;
