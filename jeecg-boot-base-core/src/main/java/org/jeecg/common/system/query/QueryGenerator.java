...
public class QueryGenerator {
	 ...
	 * 错误示例:如QueryWrapper<JeecgDemo> queryWrapper = new
	 * QueryWrapper<JeecgDemo>(jeecgDemo); <br>
	 * 正确示例:QueryWrapper<JeecgDemo> queryWrapper = new QueryWrapper<JeecgDemo>(); <br>
	...
	private static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj,
		 ...
		 * 但是不支持在自定义SQL中写orgCode in #{sys_org_code} 当一个人只有一个部门 就直接配置等于条件: orgCode 等于
		 * #{sys_org_code} 或者配置自定义SQL: orgCode = '#{sys_org_code}'
		 */
		...
		// update-begin--Author:taoyan Date:20200923 for：issues/1671
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				...
				if (ruleMap.containsKey(name)) {
					addRuleToQueryWrapper(ruleMap.get(name), column, origDescriptors[i].getPropertyType(),
							queryWrapper);
				}
				// 区间查询
				...
				if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
					...
					if (vals.length > 1) {
						queryWrapper.and(j -> {
							...
							for (int k = 1; k < vals.length; k++) {
								...
								log.info("---查询过滤器，Query规则 .or()---field:{}, rule:{}, value:{}", field, "like",
										vals[k]);
							}
							// return j;
						});
					...
					// if( (rule==null || QueryRuleEnum.EQ.equals(rule)) && "class
		...
		// update-end--Author:taoyan Date:20200923 for：issues/1671
		// 如果字段加注解了@TableField(exist = false),不走DB查询-------
	...
	private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap,
		...
		if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
			...
			// update-begin-author:taoyan date:2022-5-16 for: issues/3676
			// 获取系统用户列表时，使用SQL注入生效
			// 判断column是不是当前实体的
			...
			// update-end-author:taoyan date:2022-5-16 for: issues/3676
			// 获取系统用户列表时，使用SQL注入生效

			// update-begin-author:scott date:2022-10-10
			// for:【jeecg-boot/issues/I5FJU6】doMultiFieldsOrder() 多字段排序方法存在问题
			// 多字段排序方法没有读取 MybatisPlus 注解 @TableField 里 value 的值
			if (column.contains(",")) {
				List<String> columnList = Arrays.asList(column.split(","));
				String columnStrNew = columnList.stream()
					.map(c -> fieldColumnMap.get(c))
			...
			// update-end-author:scott date:2022-10-10
			// for:【jeecg-boot/issues/I5FJU6】doMultiFieldsOrder() 多字段排序方法存在问题
			...
			// 排序规则修改
			// 将现有排序 _ 前端传递排序条件{....,column: 'column1,column2',order: 'desc'} 翻译成sql
			// "column1,column2 desc"
			// 修改为 _ 前端传递排序条件{....,column: 'column1,column2',order: 'desc'} 翻译成sql
	...
	private static void doSuperQuery(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap,
		...
		if (parameterMap != null && parameterMap.containsKey(SUPER_QUERY_PARAMS)) {
			...
			// update-begin--Author:sunjianlei Date:20200325
			// for：高级查询的条件要用括号括起来，防止和用户的其他条件冲突 -------
			try {
				...
				// update-begin-author:sunjianlei date:20220119 for: 【JTC-573】 过滤空条件查询，防止
				// sql 拼接多余的 and
				List<QueryCondition> filterConditions = conditions.stream()
					.filter(rule -> oConvertUtils.isNotEmpty(rule.getField())
							&& oConvertUtils.isNotEmpty(rule.getRule()) && oConvertUtils.isNotEmpty(rule.getVal()))
					.collect(Collectors.toList());
				if (filterConditions.size() == 0) {
					return;
				}
				// update-end-author:sunjianlei date:20220119 for: 【JTC-573】 过滤空条件查询，防止
				// sql 拼接多余的 and
				...
				queryWrapper.and(andWrapper -> {
					for (int i = 0; i < filterConditions.size(); i++) {
						...
						if (oConvertUtils.isNotEmpty(rule.getField()) && oConvertUtils.isNotEmpty(rule.getRule())
							...
							// update-begin-author:taoyan date:20201228 for: 【高级查询】 oracle
							// 日期等于查询报错
							...
							// update-begin--author:sunjianlei date:20210702
							// for：【/issues/I3VR8E】高级查询没有类型转换，查询参数都是字符串类型 ----
							...
							// update-end-author:taoyan date:20201228 for: 【高级查询】 oracle
							// 日期等于查询报错
	...
	public static QueryRuleEnum convert2Rule(Object value) {
		// 避免空数据
		// update-begin-author:taoyan date:20210629 for: 查询条件输入空格导致return
		// null后续判断导致抛出null异常
		if (value == null) {
		...
		// update-begin--Author:scott Date:20190724 for：initQueryWrapper组装sql查询条件错误
		// #284-------------------
		...
		// update-end--Author:scott Date:20190724 for：initQueryWrapper组装sql查询条件错误
		// #284---------------------
		...
		// update-begin--Author:taoyan Date:20201229 for：initQueryWrapper组装sql查询条件错误
		// #284---------------------
		...
		// update-end--Author:taoyan Date:20201229 for：initQueryWrapper组装sql查询条件错误
		// #284---------------------
	...
	private static void addQueryByRule(QueryWrapper<?> queryWrapper, String name, String type, String value,
		...
		if (oConvertUtils.isNotEmpty(value)) {
			// update-begin--Author:sunjianlei Date:20220104
			// for：【JTC-409】修复逗号分割情况下没有转换类型，导致类型严格的数据库查询报错 -------------------
			// 针对数字类型字段，多值查询
			...
			addEasyQuery(queryWrapper, name, rule, temp);
			// update-end--Author:sunjianlei Date:20220104
			// for：【JTC-409】修复逗号分割情况下没有转换类型，导致类型严格的数据库查询报错 -------------------
		}
	}
	...
	public static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		...
		switch (rule) {
			...
			case IN:
				...
				// update-begin-author:taoyan date:20200909 for:【bug】in 类型多值查询
				// 不适配postgresql #1671
				...
				// update-end-author:taoyan date:20200909 for:【bug】in 类型多值查询 不适配postgresql
				// #1671
				break;
			case LIKE:
	...
	private static String getLikeConditionValue(Object value, QueryRuleEnum ruleEnum) {
		...
		if (str.startsWith(SymbolConstant.ASTERISK) && str.endsWith(SymbolConstant.ASTERISK)) {
			...
			if (str.indexOf(SymbolConstant.PERCENT_SIGN) >= 0) {
				if (DataBaseConstant.DB_TYPE_SQLSERVER.equals(getDbType())) {
					if (str.startsWith(SymbolConstant.SINGLE_QUOTATION_MARK)
							&& str.endsWith(SymbolConstant.SINGLE_QUOTATION_MARK)) {
						return "N" + str;
					}
					else {
	...
	private static String getTableFieldName(Class<?> clazz, String name) {
		try {
			...
			if (field == null) {
				...
				List<Field> searchFields = allFields.stream()
