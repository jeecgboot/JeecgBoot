...
public class SysDepartPermissionController extends JeecgController<SysDepartPermission, ISysDepartPermissionService> {
	...
	public Result<?> queryPageList(SysDepartPermission sysDepartPermission,
		...
		QueryWrapper<SysDepartPermission> queryWrapper = QueryGenerator.initQueryWrapper(sysDepartPermission,
				req.getParameterMap());
	...
	public Result<List<String>> queryDeptRolePermission(@RequestParam(name = "roleId", required = true) String roleId) {
		...
		try {
			List<SysDepartRolePermission> list = sysDepartRolePermissionService
				.list(new QueryWrapper<SysDepartRolePermission>().lambda()
					.eq(SysDepartRolePermission::getRoleId, roleId));
			result.setResult(list.stream()
				.map(sysDepartRolePermission -> String.valueOf(sysDepartRolePermission.getPermissionId()))
	...
	public Result<String> saveDeptRolePermission(@RequestBody JSONObject json) {
		...
		try {
			...
			// for：[VUEN-234]部门角色授权添加敏感日志------------
			LoginUser loginUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
			baseCommonService.addLog("修改部门角色ID:" + roleId + "的权限配置，操作人： " + loginUser.getUsername(),
	...
	public Result<Map<String, Object>> queryTreeListForDeptRole(
			@RequestParam(name = "departId", required = true) String departId, HttpServletRequest request) {
