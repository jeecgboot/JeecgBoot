...
public class SysPermissionController {
	...
	public Result<?> getUserPermissionByToken(HttpServletRequest request) {
		...
		try {
			...
			// 添加首页路由
			// update-begin-author:taoyan date:20200211 for: TASK #3368
			// 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
			if (!PermissionDataUtil.hasIndexPage(metaList)) {
				SysPermission indexMenu = sysPermissionService
					.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName, "首页"))
					.get(0);
				metaList.add(0, indexMenu);
			}
			// update-end-author:taoyan date:20200211 for: TASK #3368
			// 【路由缓存】首页的缓存设置有问题，需要根据后台的路由配置来实现是否缓存
			...
			// update-end---author:liusq ---date:2022-06-29
			// for：接口返回值修改，同步修改这里的判断逻辑-----------
			// update-end--Author:zyf Date:20220425 for：自定义首页地址 LOWCOD-1578

			if (roleIndex != null) {
				List<SysPermission> menus = metaList.stream()
					.filter(sysPermission -> "首页".equals(sysPermission.getName()))
					.collect(Collectors.toList());
				// update-begin---author:liusq ---date:2022-06-29
				// for：设置自定义首页地址和组件----------
	...
	private boolean isWwwHttpUrl(String url) {
		boolean flag = url != null
				&& (url.startsWith(CommonConstant.HTTP_PROTOCOL) || url.startsWith(CommonConstant.HTTPS_PROTOCOL)
						|| url.startsWith(SymbolConstant.DOUBLE_LEFT_CURLY_BRACKET));
		if (flag) {
			return true;
		}
		return false;
	...
	public Result<List<String>> queryDepartPermission(
		...
		try {
			...
			result.setResult(list.stream()
				.map(sysDepartPermission -> String.valueOf(sysDepartPermission.getPermissionId()))
				.collect(Collectors.toList()));
			result.setSuccess(true);
		}
		catch (Exception e) {
