 ...
 */
@Service
public class SysTenantPackServiceImpl extends ServiceImpl<SysTenantPackMapper, SysTenantPack>
	...
	public void editPackPermission(SysTenantPack sysTenantPack) {
		...
		if (oConvertUtils.isEmpty(permissionIds)) {
			...
			if (oConvertUtils.isNotEmpty(oldPermissionIds)) {
				...
				List<String> permissionList = oldPermissionIds.stream()
				...
				List<String> permissionAddList = Arrays.stream(permissionIds.split(SymbolConstant.COMMA))
	...
	public void addDefaultTenantPack(Integer tenantId) {
		...
		ISysTenantPackService currentService = SpringContextUtils.getApplicationContext()
			.getBean(ISysTenantPackService.class);
