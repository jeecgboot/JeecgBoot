...
public class ThirdAppWechatEnterpriseServiceImpl implements IThirdAppService {
	 ...
	 * @param accessToken
	 */
	private void syncDepartmentRecursion(List<SysDepartTreeModel> sysDepartsTree, List<Department> departments,
			Department parent, String accessToken) {
	 ...
	 * 递归同步部门到本地
	 */
	private void syncDepartmentToLocalRecursion(List<JwDepartmentTreeVo> departmentTreeList, String sysParentId,
			String username, SyncInfoVo syncInfo) {
	...
	public SyncInfoVo syncLocalUserToThirdApp(String ids) {
		...
		// 获取企业微信所有的用户
		// List<User> qwUsers = JwUserAPI.getDetailUsersByDepartid("1", null, null,
		// accessToken);
		...
		for1: for (SysUser sysUser : sysUsers) {
			 ...
			 * 本地表里没有，就先用手机号判断，不通过再用username判断。
			 */
			User qwUser;
			...
			for (User qwUserTemp : qwUsers) {
				if (sysThirdAccount == null || oConvertUtils.isEmpty(sysThirdAccount.getThirdUserId())
						|| !sysThirdAccount.getThirdUserId().equals(qwUserTemp.getUserid())) {
					// sys_third_account 表匹配失败，尝试用手机号匹配
					// 新版企业微信调整了API，现在只能通过userid来判断是否同步过了
					// String phone = sysUser.getPhone();
					// if (!(oConvertUtils.isEmpty(phone) ||
					// phone.equals(qwUserTemp.getMobile()))) {
	...
	public SyncInfoVo syncThirdAppUserToLocal() {
		...
		for (User qwUser : qwUsersList) {
			 ...
			 * 本地表里没有，就先用手机号判断，不通过再用username判断。
			 */
			SysThirdAccount sysThirdAccount = sysThirdAccountService.getOneByThirdUserId(qwUser.getUserid(),
					THIRD_TYPE);
			List<SysUser> collect = sysUsersList.stream()
				.filter(user -> (qwUser.getMobile().equals(user.getPhone())
						|| qwUser.getUserid().equals(user.getUsername())))
	...
	private User sysUserToQwUser(SysUser sysUser, User user) {
		...
		if (sysUser.getStatus() != null) {
			if (CommonConstant.USER_UNFREEZE.equals(sysUser.getStatus())
					|| CommonConstant.USER_FREEZE.equals(sysUser.getStatus())) {
	...
	private String getTouser(String origin, boolean toAll) {
		if (toAll) {
			...
			List<SysThirdAccount> thirdAccountList = sysThirdAccountService.listThirdUserIdByUsername(toUsers,
					THIRD_TYPE);
			List<String> toUserList = thirdAccountList.stream()
				.map(SysThirdAccount::getThirdUserId)
				.collect(Collectors.toList());
			// 多个接收者用‘|’分隔
