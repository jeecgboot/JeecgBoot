	...
	public void pushMessage(String userId, String message) {
		for (Map.Entry<String, Session> item : sessionPool.entrySet()) {
			...
			if (item.getKey().contains(userId)) {
				...
				try {
					// update-begin-author:taoyan date:20211012 for: websocket报错
					// https://gitee.com/jeecg/jeecg-boot/issues/I4C0MU
					synchronized (session) {
						log.info("【系统 WebSocket】推送单人消息:" + message);
						session.getBasicRemote().sendText(message);
					}
					// update-end-author:taoyan date:20211012 for: websocket报错
					// https://gitee.com/jeecg/jeecg-boot/issues/I4C0MU
				}
				catch (Exception e) {
