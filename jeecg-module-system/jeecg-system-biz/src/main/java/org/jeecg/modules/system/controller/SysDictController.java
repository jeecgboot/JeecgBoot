...
public class SysDictController {
	...
	public Result<List<DictModel>> getDictItems(@PathVariable("dictCode") String dictCode,
		...
		// update-begin-author:taoyan date:20220317 for:
		// VUEN-222【安全机制】字典接口、online报表、online图表等接口，加一些安全机制
		if (!dictQueryBlackListHandler.isPass(dictCode)) {
			return result.error500(dictQueryBlackListHandler.getError());
		}
		// update-end-author:taoyan date:20220317 for:
		// VUEN-222【安全机制】字典接口、online报表、online图表等接口，加一些安全机制
		try {
	...
	/**
	 * 【接口签名验证】 【JSearchSelectTag下拉搜索组件专用接口】 大数据量的字典表 走异步加载 即前端输入内容过滤数据
	...
	/**
	 * 【接口签名验证】 【给表单设计器的表字典使用】下拉搜索模式，有值时动态拼接数据
	 * @param dictCode
	...
	public Result<List<DictModel>> loadDictOrderByValue(@PathVariable("dictCode") String dictCode,
	...
	public Result<List<String>> loadDictItem(@PathVariable("dictCode") String dictCode,
			@RequestParam(name = "key") String keys, @RequestParam(value = "sign", required = false) String sign,
			@RequestParam(value = "delNotExist", required = false, defaultValue = "true") boolean delNotExist,
	...
	/**
	 * 【接口签名验证】 根据表名——显示字段-存储字段 pid 加载树形数据
	 * @param hasChildField 是否叶子节点字段
	 * @param converIsLeafVal 是否需要系统转换 是否叶子节点的值 (0标识不转换、1标准系统自动转换)
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/loadTreeData", method = RequestMethod.GET)
	public Result<List<TreeSelectModel>> loadTreeData(@RequestParam(name = "pid", required = false) String pid,
				...
				for (int i = 0; i < list.size(); i++) {
					...
					try {
						...
						if (integer > 0) {
							...
							errorLines++;
							int lineNumber = i + 1;
							// update-begin---author:wangshuai ---date:20220209
							// for：[JTC-1168]字典编号不能为空------------
							...
							// update-end---author:wangshuai ---date:20220209
							// for：[JTC-1168]字典编号不能为空------------
						}
					}
					catch (Exception e) {
