...
public class LoginController {
	...
	public Result<JSONObject> login(@RequestBody SysLoginModel sysLoginModel) {
		...
		if (checkCode == null || !checkCode.toString().equals(lowerCaseCaptcha)) {
			log.warn("验证码错误，key= {} , Ui checkCode= {}, Redis checkCode = {}", sysLoginModel.getCheckKey(),
					lowerCaseCaptcha, checkCode);
	...
	public Result<JSONObject> getUserInfo(HttpServletRequest request) {
		...
		if (oConvertUtils.isNotEmpty(username)) {
			...
			// update-begin---author:scott ---date:2022-06-20
			// for：vue3前端，支持自定义首页-----------
			...
			if (oConvertUtils.isNotEmpty(version) && roleIndex != null
					&& oConvertUtils.isNotEmpty(roleIndex.getUrl())) {
	...
	public Result<String> sms(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			// update-begin-author:taoyan date:2022-9-13 for: VUEN-2245
			// 【漏洞】发现新漏洞待处理20220906
			// 验证码10分钟内有效
	...
	private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result) {
		...
		if (departs == null || departs.size() == 0) {
			...
			// 查询当前是否有登录部门
			// update-begin--Author:wangshuai Date:20200805
			// for：如果用戶为选择部门，数据库为存在上一次登录部门，则取一条存进去
			SysUser sysUserById = sysUserService.getById(sysUser.getId());
			if (oConvertUtils.isEmpty(sysUserById.getOrgCode())) {
				sysUserService.updateUserDepart(username, departs.get(0).getOrgCode(), null);
			}
			// update-end--Author:wangshuai Date:20200805
			// for：如果用戶为选择部门，数据库为存在上一次登录部门，则取一条存进去
	...
	public Result<JSONObject> mLogin(@RequestBody SysLoginModel sysLoginModel) throws Exception {
		...
		if (oConvertUtils.isEmpty(orgCode)) {
			...
			List<SysDepart> departs = sysDepartService.queryUserDeparts(sysUser.getId());
			// update-begin-author:taoyan date:20220117 for:
			// JTC-1068【app】新建用户，没有设置部门及角色，点击登录提示暂未归属部，一直在登录页面 使用手机号登录 可正常
			if (departs == null || departs.size() == 0) {
				/*
				 * result.error500("用户暂未归属部门,不可登录!"); return result;
				 */
			}
			else {
			...
			// update-end-author:taoyan date:20220117 for:
			// JTC-1068【app】新建用户，没有设置部门及角色，点击登录提示暂未归属部，一直在登录页面 使用手机号登录 可正常
		}
