...
public class CategoryCodeRule implements IFillRuleHandler {
	...
	public Object execute(JSONObject params, JSONObject formData) {
		...
		/*
		 * 分成三种情况 1.数据库无数据 调用YouBianCodeUtil.getNextYouBianCode(null); 2.添加子节点，无兄弟元素
		 * YouBianCodeUtil.getSubYouBianCode(parentCode,null); 3.添加子节点有兄弟元素
		 * YouBianCodeUtil.getNextYouBianCode(lastCode);
		 */
		// 找同类 确定上一个最大的code值
		LambdaQueryWrapper<SysCategory> query = new LambdaQueryWrapper<SysCategory>()
			.eq(SysCategory::getPid, categoryPid)
			.isNotNull(SysCategory::getCode)
