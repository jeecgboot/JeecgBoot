...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	public LoginUser getUserByName(String username) {
		// update-begin-author:taoyan date:2022-6-6 for: VUEN-1276 【v3流程图】测试bug
		// 1、通过我发起的流程或者流程实例，查看历史，流程图预览问题
		if (oConvertUtils.isEmpty(username)) {
			return null;
		}
		// update-end-author:taoyan date:2022-6-6 for: VUEN-1276 【v3流程图】测试bug
		// 1、通过我发起的流程或者流程实例，查看历史，流程图预览问题
	...
	@Override
	public List<SysPermissionDataRuleModel> queryPermissionDataRule(String component, String requestPath,
			String username) {
		...
		if (oConvertUtils.isNotEmpty(component)) {
			...
			if (currentSyspermission == null || currentSyspermission.size() == 0) {
				...
				// update-begin-author:taoyan date:20211027 for: online菜单如果配置成一级菜单 权限查询不到
				// 取消menuType = 1
				// queryQserMatch.eq(SysPermission::getMenuType, 1);
				// update-end-author:taoyan date:20211027 for: online菜单如果配置成一级菜单 权限查询不到
				// 取消menuType = 1
			...
			if (currentSyspermission == null || currentSyspermission.size() == 0) {
				...
				if (regUrl != null) {
					currentSyspermission = sysPermissionMapper
						.selectList(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getMenuType, 2)
							.eq(SysPermission::getUrl, regUrl)
		...
		if (currentSyspermission != null && currentSyspermission.size() > 0) {
			...
			for (SysPermission sysPermission : currentSyspermission) {
				// update-begin--Author:scott Date:20191119 for：数据权限规则编码不规范，项目存在相同包名和类名
				// #722
				List<SysPermissionDataRule> temp = sysPermissionDataRuleService.queryPermissionDataRules(username,
						sysPermission.getId());
	...
	/**
	 * 匹配前端传过来的地址 匹配成功返回正则地址 AntPathMatcher匹配地址 ()* 匹配0个或多个字符 ()**匹配0个或多个目录
	 */
	...
	public List<DictModel> queryTableDictItemsByCode(String table, String text, String code) {
		// update-begin-author:taoyan date:20200820 for:【Online+系统】字典表加权限控制机制逻辑，想法不错
		// LOWCOD-799
		if (table.indexOf(SymbolConstant.SYS_VAR_PREFIX) >= 0) {
			table = QueryGenerator.getSqlRuleValue(table);
		}
		// update-end-author:taoyan date:20200820 for:【Online+系统】字典表加权限控制机制逻辑，想法不错
		// LOWCOD-799
	...
	public void sendSysAnnouncement(MessageDTO message) {
		this.sendSysAnnouncement(message.getFromUser(), message.getToUser(), message.getTitle(), message.getContent(),
				message.getCategory());
		try {
			// 同步发送第三方APP消息
	...
	public void sendBusAnnouncement(BusMessageDTO message) {
		sendBusAnnouncement(message.getFromUser(), message.getToUser(), message.getTitle(), message.getContent(),
				message.getCategory(), message.getBusType(), message.getBusId());
		try {
			// 同步发送第三方APP消息
			wechatEnterpriseService.sendMessage(message, true);
			dingtalkService.sendMessage(message, true);
			...
			try {
				...
				if (dbType.indexOf(DataBaseConstant.DB_TYPE_MYSQL.toLowerCase()) >= 0) {
					...
					DB_TYPE = DataBaseConstant.DB_TYPE_ORACLE;
						...
						|| dbType.indexOf(DataBaseConstant.DB_TYPE_SQL_SERVER_BLANK) >= 0) {
	...
	public DynamicDataSourceModel getDynamicDbSourceByCode(String dbSourceCode) {
		SysDataSource dbSource = dataSourceService
			.getOne(new LambdaQueryWrapper<SysDataSource>().eq(SysDataSource::getCode, dbSourceCode));
		if (dbSource != null && StringUtils.isNotBlank(dbSource.getDbPassword())) {
			String dbPassword = dbSource.getDbPassword();
			String decodedStr = SecurityUtil.jiemi(dbPassword);
			dbSource.setDbPassword(decodedStr);
	...
	public List<String> queryDeptUsersByUserId(String userId) {
		...
		List<SysUserDepart> userDepartList = sysUserDepartService
			.list(new QueryWrapper<SysUserDepart>().eq("user_id", userId));
		if (userDepartList != null) {
			...
			if (oConvertUtils.isNotEmpty(orgCodes)) {
				...
				List<SysUserDepart> userList = sysUserDepartService
					.list(new QueryWrapper<SysUserDepart>().in("dep_id", listIds));
	 ...
	 */
	private void sendSysAnnouncement(String fromUser, String toUser, String title, String msgContent,
	 ...
	 * @param busId
	 */
	private void sendBusAnnouncement(String fromUser, String toUser, String title, String msgContent,
	...
	/**
	 * 【下拉搜索】 大数据量的字典表 走异步加载，即前端输入内容过滤数据
