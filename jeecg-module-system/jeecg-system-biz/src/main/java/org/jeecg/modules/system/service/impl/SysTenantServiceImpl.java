...
public class SysTenantServiceImpl extends ServiceImpl<SysTenantMapper, SysTenant> implements ISysTenantService {
	...
	public Result<String> invitationUser(String phone) {
		...
		if (count > 0) {
			result.setSuccess(false);
			result.setMessage("用户名(" + userByPhone.getRealname() + ")" + "手机号(" + userByPhone.getPhone() + ")"
					+ "已存在该租户中,不能重复邀请");
			return result;
		}
	...
	public void addBatchTenantPackUser(SysTenantPackUser sysTenantPackUser) {
		...
		if (oConvertUtils.isNotEmpty(userIds)) {
			ISysTenantService currentService = SpringContextUtils.getApplicationContext()
				.getBean(ISysTenantService.class);
	...
	/**
	 * 获取租户下 某个产品包的用户 或者是 租户下产品包的申请用户
	 * @param tenantId
	 * @param packId
