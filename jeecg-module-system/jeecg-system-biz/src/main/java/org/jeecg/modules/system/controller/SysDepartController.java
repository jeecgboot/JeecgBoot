...
public class SysDepartController {
	...
	public Result<List<SysDepartTreeModel>> queryMyDeptTreeList() {
		...
		try {
			if (oConvertUtils.isNotEmpty(user.getUserIdentity())
				...
				// update-begin--Author:liusq Date:20210624 for:部门查询ids为空后的前端显示问题
				// issues/I3UD06
				...
				// update-end--Author:liusq Date:20210624 for:部门查询ids为空后的前端显示问题
				// issues/I3UD06
	...
	public Result queryParentIds(@RequestParam(name = "departId", required = false) String departId,
	...
	/**
	 * 通过excel导入数据 部门导入方案1: 通过机构编码来计算出部门的父级ID,维护上下级关系; 部门导入方案2:
	 * 你也可以改造下程序,机构编码直接导入,先不设置父ID;全部导入后,写一个sql,补下父ID;
	 * @param request
	 * @param response
	 * @return
	 */
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				...
				for (SysDepart sysDepart : listSysDeparts) {
					...
					// update-begin---author:liusq Date:20210223 for：批量导入部门以后，不能追加下一级部门
					// #2245------------
					sysDepart.setOrgType(sysDepart.getOrgCode().length() / codeLength + "");
					// update-end---author:liusq Date:20210223 for：批量导入部门以后，不能追加下一级部门
					// #2245------------
					sysDepart.setDelFlag(CommonConstant.DEL_FLAG_0.toString());
					// update-begin---author:wangshuai ---date:20220105 for：[JTC-363]部门导入
					// 机构类别没有时导入失败，赋默认值------------
					if (oConvertUtils.isEmpty(sysDepart.getOrgCategory())) {
						sysDepart.setOrgCategory("1");
					}
					// update-end---author:wangshuai ---date:20220105 for：[JTC-363]部门导入
					// 机构类别没有时导入失败，赋默认值------------
				...
				return ImportExcelUtil.imporReturnRes(errorMessageList.size(),
						listSysDeparts.size() - errorMessageList.size(), errorMessageList);
			}
			catch (Exception e) {
				log.error(e.getMessage(), e);
