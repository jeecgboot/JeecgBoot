...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	public Result<IPage<SysUser>> queryPageList(HttpServletRequest req, QueryWrapper<SysUser> queryWrapper,
		...
		if (oConvertUtils.isNotEmpty(departId)) {
			...
			// update-end---author:wangshuai ---date:20220322
			// for：[issues/I4XTYB]查询用户时，当部门id 下没有分配用户时接口报错------------
		}
		// 用户ID
	...
	// ----sql注入 方法没有使用，注掉
	/*
	 * @Override public IPage<SysUser> getUserByDepartIdAndQueryWrapper(Page<SysUser>
	 * page, String departId, QueryWrapper<SysUser> queryWrapper) {
			...
			for (SysUserDepart depart : userDepartList) {
				...
				if (!Arrays.asList(arr).contains(depart.getDepId())) {
					List<SysDepartRole> sysDepartRoleList = sysDepartRoleMapper
						.selectList(new QueryWrapper<SysDepartRole>().lambda()
	...
	public void saveUser(SysUser user, String selectedRoles, String selectedDeparts, String relTenantIds) {
		...
		// update-begin---author:wangshuai ---date:20230112 for：用户创建的时候增加临时角色
		// test------------
		// 开启租户saas模式
		...
		// update-end---author:wangshuai ---date:20230112 for：用户创建的时候增加临时角色
		// test------------
	...
	public void userQuit(String username) {
		...
		// update-end---author:wangshuai ---date:20230111
		// for：[QQYUN-3951]租户用户离职重构------------
		// 触发离职流程
		LoginUser userInfo = new LoginUser();
	...
	public Result<JSONObject> setLoginTenant(SysUser sysUser, JSONObject obj, String username,
		...
		if (tenantList != null && tenantList.size() > 0) {
			if (tenantList.size() == 1) {
				...
				List<SysTenant> listAfterFilter = tenantList.stream()
	...
	private void editUserTenants(String userId, String relTenantIds) {
		...
		if (oConvertUtils.isEmpty(relTenantIds)) {
			...
			if (oConvertUtils.isNotEmpty(oldTenantIds)) {
				...
				List<Integer> tenantIdList = oldTenantIds.stream()
					.filter(item -> !relTenantIds.contains(item.toString()))
					.collect(Collectors.toList());
				for (Integer tenantId : tenantIdList) {
					this.deleteTenantByUserId(userId, tenantId);
				}
				// 找到原来租户的用户id与新的租户id不同之处，进行新增
				String tenantIds = Arrays.stream(relTenantIds.split(SymbolConstant.COMMA))
	...
	public void batchEditUsers(JSONObject json) {
		...
		if (oConvertUtils.isNotEmpty(selecteddeparts)) {
			...
			LambdaQueryWrapper<SysDepart> departQuery = new LambdaQueryWrapper<SysDepart>().eq(SysDepart::getTenantId,
					currentTenantId);
	...
	public DepartAndUserInfo searchByKeyword(String keyword) {
		...
		if (oConvertUtils.isNotEmpty(keyword)) {
			LambdaQueryWrapper<SysUser> query1 = new LambdaQueryWrapper<SysUser>().like(SysUser::getRealname, keyword);
			...
			LambdaQueryWrapper<SysDepart> query2 = new LambdaQueryWrapper<SysDepart>().like(SysDepart::getDepartName,
					keyword);
			if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL) {
	...
	public UpdateDepartInfo getUpdateDepartInfo(String departId) {
		...
		if (depart != null) {
			...
			// 获取部门负责人信息
			LambdaQueryWrapper<SysUser> query = new LambdaQueryWrapper<SysUser>().eq(SysUser::getUserIdentity, 2)
	...
	public void doUpdateDepartInfo(UpdateDepartInfo info) {
		...
		if (depart != null) {
			...
			List<String> departChargeUserIdList = departChargeUsers.stream()
				.map(i -> i.getId())
				.collect(Collectors.toList());
			// 修改部门负责人
	...
	private List<SysUser> queryDepartChargePersons(String departId) {
		...
		LambdaQueryWrapper<SysUserDepart> query1 = new LambdaQueryWrapper<SysUserDepart>().eq(SysUserDepart::getDepId,
				departId);
		...
		if (list1 != null && list1.size() > 0) {
			...
			LambdaQueryWrapper<SysUser> query2 = new LambdaQueryWrapper<SysUser>().in(SysUser::getId, userIdList);
	...
	public void updateTenantDepart(SysUser user, String tenantId, String departs) {
		...
		if (oConvertUtils.isNotEmpty(departs)) {
			// 获取当前租户下的部门id,根据前台
			departList = sysUserDepartMapper.getTenantDepart(Arrays.asList(departs.split(SymbolConstant.COMMA)),
					tenantId);
		}
		// 查询当前租户下部门和用户已关联的部门
