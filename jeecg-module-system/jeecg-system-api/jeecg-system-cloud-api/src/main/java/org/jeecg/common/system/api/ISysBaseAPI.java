...
/**
 *
 * 1、cloud接口数量43 local：35 common：9 额外一个特殊queryAllRole一个当两个用 - 相比较local版 -
 * 去掉了一些方法：addLog、getDatabaseType、queryAllDepart、queryAllUser(Wrapper
 * wrapper)、queryAllUser(String[] userIds, int pageNo, int pageSize) -
 * 修改了一些方法：createLog、sendSysAnnouncement（只保留了一个，其余全部干掉）
 * 2、@ConditionalOnMissingClass("org.jeecg.modules.system.service.impl.SysBaseApiImpl")=>
 * 有实现类的时候，不实例化Feign接口
 *
 * @author: jeecg-boot
 */
@Component
@FeignClient(contextId = "sysBaseRemoteApi", value = ServiceNameConstants.SERVICE_SYSTEM,
...
public interface ISysBaseAPI extends CommonAPI {
	...
	List<DictModel> queryTableDictItemsByCode(@RequestParam("table") String table, @RequestParam("text") String text,
	...
	List<DictModel> queryFilterTableDictInfo(@RequestParam("table") String table, @RequestParam("text") String text,
			@RequestParam("code") String code, @RequestParam("filterSql") String filterSql);
	...
	public List<String> queryTableDictByKeys(@RequestParam("table") String table, @RequestParam("text") String text,
			@RequestParam("code") String code, @RequestParam("keyArray") String[] keyArray);
	...
	String translateDictFromTable(@RequestParam("table") String table, @RequestParam("text") String text,
			@RequestParam("code") String code, @RequestParam("key") String key);
	...
	List<SysPermissionDataRuleModel> queryPermissionDataRule(@RequestParam("component") String component,
			@RequestParam("requestPath") String requestPath, @RequestParam("username") String username);
	...
	void sendEmailMsg(@RequestParam("email") String email, @RequestParam("title") String title,
	...
	/**
	 * 46 【JSearchSelectTag下拉搜索组件专用接口】 大数据量的字典表 走异步加载 即前端输入内容过滤数据
	...
	List<DictModel> loadDictItemByKeyword(@RequestParam("dictCode") String dictCode,
	...
	Map<String, List<DictModel>> translateManyDict(@RequestParam("dictCodes") String dictCodes,
	...
	List<DictModel> translateDictFromTableByKeys(@RequestParam("table") String table, @RequestParam("text") String text,
			@RequestParam("code") String code, @RequestParam("keys") String keys);
