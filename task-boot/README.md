...
- 在线演示 ：  [http://boot.jeecg.com](http://boot.task.com)

- 在线文档：  [http://doc.jeecg.com/1273753](http://doc.task.com/1273753)
...
```
	...
	public Result<IPage<TaskDemo>> list(TaskDemo taskDemo, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, 
	                                     @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
			HttpServletRequest req) {
		Result<IPage<TaskDemo>> result = new Result<IPage<TaskDemo>>();
...
> 功能说明：   一键生成的代码（包括：controller、service、dao、mapper、entity、vue）
 ...
 - 模板位置： src/main/resources/task/code-template
 - 技术文档： http://doc.task.com/1273927
...
# 4.配置host
    ...
    # taskboot
    127.0.0.1   task-boot-redis
    127.0.0.1   task-boot-mysql
    127.0.0.1   task-boot-system

# 5.启动镜像__容器组（也可取代运行中的镜像）
    docker-compose up -d

# 6.访问后台项目（注意要开启swagger）
    http://localhost:8080/task-boot/doc.html
``` 
