...
public class PaysController extends BaseController {
    ...
    public PageResult<PaysVo> page() {
        ...
        if (paysList.isEmpty()) { // 如果没有支付配置，则拷贝一份默认配置
            pageParam.put("username","guest");
            paysList = paysService.list(pageParam.getOrderWrapper());
            for (Pays pays : paysList) {
                pays.setId(null);
            }
            saveBatch(paysList);
            pageParam.put("username",getLoginUsername());
            paysList = paysService.page(pageParam, pageParam.getWrapper()).getRecords();
        }
    ...
    public JsonResult save(Pays pays) {
        pays.setUsername(getLoginUsername());
