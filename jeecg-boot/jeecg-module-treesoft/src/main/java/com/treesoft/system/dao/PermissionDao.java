package com.treesoft.system.dao;

import com.alibaba.fastjson.JSONArray;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.treesoft.common.utils.DateUtil;
import com.treesoft.system.entity.Config;
import com.treesoft.system.persistence.Page;
import com.treesoft.system.utils.CryptoUtil;
import lombok.Cleanup;
import lombok.extern.slf4j.Slf4j;
import org.apache.commons.lang3.StringUtils;
import com.treesoft.system.core.BusiDataBaseUtil;
...
public class PermissionDao {
    ...
    public boolean configUpdate(Config config) {
        ...
        if (StringUtils.isNotBlank(id)) {
            if (StringUtils.isNotBlank(config.getPassword())) {
                ...
                bl = jdbcTemplate.update(sql,
                        ...
                        config.getUserName(),
                        CryptoUtil.encode(String.valueOf(config.getUserName()) + "`" + config.getPassword()),
                        isDefault,
                        config.getName(),
                        config.getPort(),
                        config.getIp(),
        ...
        jdbcTemplate.update(updateSQL, ids.substring(0, ids.length() - 1));
        return bl;
    }
    ...
    public String getViewSqlForOracle(String databaseName, String tableName, String databaseConfigId)
        ...
        if (list.size() == 1) {
            Map<String, Object> map = list.get(0);
            str = "create view " + tableName + " as " + map.get("TEXT") + ";";
        }
        return str;
    }
    ...
    public boolean restoreDBFromFile(String databaseName, String fpath, String databaseConfigId)
        ...
        try {
            reader = new BufferedReader(new InputStreamReader(new FileInputStream(fpath), StandardCharsets.UTF_8));
