...
const dataId = ref('')
const backRouteName = ref('StudentList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/student/student/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/student/student/edit':'/student/student/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
