...
<template>
  <PageLayout navTitle="课程管理模块"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './CursorstudentData';
defineOptions({
  name: 'CursorstudentList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/cursorstudent/cursorstudent/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/cursorstudent/cursorstudent/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
