...
layout: 'default',
style: {
navigationBarTitleText: '成绩管理模块',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="成绩管理模块"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './ScoremanagementData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/scoremanagement/scoremanagement/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/scoremanagement/scoremanagement/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
