...
layout: 'default',
style: {
navigationBarTitleText: '学生表',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="学生表"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './StudentData';
defineOptions({
  name: 'StudentList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/student/student/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/student/student/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
