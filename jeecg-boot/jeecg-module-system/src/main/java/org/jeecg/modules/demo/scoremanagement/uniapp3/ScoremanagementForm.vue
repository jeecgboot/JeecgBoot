      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['examname']"
                  :label="get4Label('考试名字')"
                  name='examname'
                  prop='examname'
                  placeholder="请选择考试名字"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <DateTime
                  :label="get4Label('考试时间')"
                  labelWidth="100px"
                  type="datetime"
                  format="YYYY-MM-DD HH:mm:ss"
                  name='examtime'
                  v-model="myFormData['examtime']"
              ></DateTime>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['studentscore']"
                   :label="get4Label('学生成绩')"
                   name='studentscore'
                   prop='studentscore'
                   placeholder="请选择学生成绩"
                   inputMode="numeric"
...
const dataId = ref('')
const backRouteName = ref('ScoremanagementList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/scoremanagement/scoremanagement/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
// 提交表单
const handleSubmit = () => {
  let url = dataId.value?'/scoremanagement/scoremanagement/edit':'/scoremanagement/scoremanagement/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
