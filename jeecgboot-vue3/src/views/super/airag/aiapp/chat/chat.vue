<template>
  <div class="chatWrap">
    <div class="content">
      ...
      <div class="footer">
        <div class="topArea">
          <presetQuestion @out-question="handleOutQuestion" :quickCommandData="quickCommandData" />
        </div>
              ...
              >
                <a-tooltip title="图片上传，支持jpg/jpeg/png">
                  <a-button class="sendBtn" type="text">
                    <Icon icon="ant-design:picture-outlined" style="color: rgba(15, 21, 40, 0.8)" />
                  </a-button>
                </a-tooltip>
              </a-upload>
              <a-divider v-if="!loading" type="vertical" style="border-color: #38374314" />
              <a-button
                @click="
                  () => {
                    handleSubmit();
...
<script setup lang="ts">
  ...
  async function handleStopChat() {
    ...
    console.log('ai 聊天：：：---停止消息，当前requestId:', currentRequestId);
    if (currentRequestId) {
      try {
        //调用后端接口停止响应
        await defHttp.get(
  ...
  async function sendMessage(message, options) {
    ...
    const readableStream = await defHttp
      .post(
        {
          url: props.url,
          params: param,
