...
public class VxeMockController {
	 ...
	 */
	@GetMapping("/getData")
	public Result getMockData(@RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
	...
	public Result getMockDdjhData(
			...
			MockEntity mockEntity, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
			@RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
			// 父级id，根据父级id查询子级，如果为空则查询顶级
			@RequestParam(name = "parentId", required = false) String parentId,
		...
		/*
		 * 注：实际使用中不用写上面那种繁琐的代码，这里只是为了直观的输出到控制台里而写的示例， 使用下面这种写法更简洁方便
		 */
